...
function initializeTests(
  ...
  supportedTargetPlatforms.forEach(function (supportedPlatform) {
    if (
      testUtil_1.TestUtil.readMochaCommandLineFlag(
        supportedPlatform.getCommandLineFlagName()
      )
    )
    ...
    it("sets up tests correctly", function (done) {
      ...
      targetPlatforms.forEach(function (platform) {
        promises.push(
          platform
            .getEmulatorManager()
            .bootEmulator(TestConfig.restartEmulators)
        );
      });
      console.log("Building test project.");
      // create the test project
      promises.push(
        createTestProject(TestConfig.testRunDirectory)
          .then(function () {
    ...
    describe("CodePush", function () {
      before(function () {
        ...
        return targetPlatform
          ...
          .then(
            projectManager.preparePlatform.bind(
              projectManager,
      ...
      after(function () {
        ...
        return projectManager
          .cleanupAfterPlatform(TestConfig.testRunDirectory, targetPlatform)
          .then(
            projectManager.cleanupAfterPlatform.bind(
              projectManager,
              TestConfig.updatesDirectory,
              targetPlatform
            )
  ...
  // BEGIN TESTING //
  describe(
    "CodePush " + projectManager.getPluginName() + " Plugin",
    function () {
      ...
      if (TestConfig.shouldSetup)
        ...
        targetPlatforms.forEach(function (platform) {
          var prefix =
            (TestConfig.onlyRunCoreTests ? "Core Tests " : "Tests ") +
            TestConfig.thisPluginPath +
            " on ";
          describe(prefix + platform.getName(), function () {
            return createAndRunTests(platform);
          });
        });
      }
    }
